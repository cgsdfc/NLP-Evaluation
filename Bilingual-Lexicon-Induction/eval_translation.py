...
def main():
    ...
    parser.add_argument('--type', type=int,help='my dictionary type')
    ...
    if args.type ==1:
        ...
        for line in f:
            ...
            src_tmp = [item for item in src.split() if len(item)>=1]
            trg_tmp = [item for item in trg.split() if len(item)>=1]
            for src_item in src_tmp:
                for trg_item in trg_tmp:
                    try:
                        src_ind = src_word2ind[src_item]
                        trg_ind = trg_word2ind[trg_item]
                        src2trg[src_ind].add(trg_ind)
                        vocab.add(src_item)
                    except KeyError:
                        oov.add(src_item)
